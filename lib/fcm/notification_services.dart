...
Future<void> firebaseMessagingBackgroundHandler(RemoteMessage message) async {
  try {
    ...
    log('Background message received: ${message.messageId}', name: 'NotificationServices');
  } catch (error, stackTrace) {
    log('Failed to process background message.', name: 'NotificationServices', error: error, stackTrace: stackTrace);
  }
}
...
class NotificationServices {
  ...
  static Future<String> getAccessToken() async {
    final serviceAccountJson = {
      ...
      "client_email": "firebase-adminsdk-fbsvc@nwl-bodyworncamera-2026.iam.gserviceaccount.com",
  ...
  Future<void> initNotifications() async {
    ...
    try {
      ...
      log('Failed to initialize notifications.', name: 'NotificationServices', error: error, stackTrace: stackTrace);
    }
  }

  Future<void> showTestNotification({String title = 'แจ้งเตือนการตรวจจับ', String body = ''}) async {
    if (!_isInitialized) {
    ...
    await _localNotifications.show(
      ...
      payload: jsonEncode({'route': Routes.NOTI_LIST, 'data': <String, dynamic>{}}),
    );
  }
  ...
  Future<void> _requestFcmPermission() async {
    if (kIsWeb || _isApplePlatform) {
      ...
      log('FCM permission: ${setting.authorizationStatus}', name: 'NotificationServices');
  ...
  Future<void> _initLocalNotifications() async {
    ...
    await _localNotifications.initialize(initializationSettings, onDidReceiveNotificationResponse: _onLocalNotificationTapped);
    ...
    if (_isApplePlatform) {
      final darwinPlugin = _localNotifications.resolvePlatformSpecificImplementation<IOSFlutterLocalNotificationsPlugin>();
  ...
  void _registerForegroundMessageHandler() {
    FirebaseMessaging.onMessage.listen((message) async {
      log('Foreground message received: ${message.messageId}', name: 'NotificationServices');
  ...
  Future<void> _logCurrentToken() async {
    try {
      ...
      log('Failed to fetch FCM token.', name: 'NotificationServices', error: error, stackTrace: stackTrace);
    } catch (error, stackTrace) {
      log('Failed to fetch FCM token.', name: 'NotificationServices', error: error, stackTrace: stackTrace);
    }
  }
  ...
  Future<void> _showForegroundNotification(RemoteMessage message) async {
    ...
    await _localNotifications.show(
      ...
      body,
      _notificationDetails,
      payload: jsonEncode({'route': _resolveRoute(message.data), 'data': message.data}),
    );
  }
  ...
  void _onLocalNotificationTapped(NotificationResponse response) {
    ...
    try {
      ...
      log('Failed to parse local notification payload.', name: 'NotificationServices', error: error, stackTrace: stackTrace);
      _navigateToRoute(route: Routes.NOTI_LIST);
    }
  }

  void _handleRemoteMessageNavigation(RemoteMessage message) {
    _navigateToRoute(route: _resolveRoute(message.data), arguments: message.data);
  }

  void _navigateToRoute({required String route, Map<String, dynamic>? arguments}) {
