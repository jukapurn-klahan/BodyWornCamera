...
class DropdownSearchState<T> extends State<DropdownSearch<T>> {
  ...
  Widget _defaultSelectedItemWidget() {
    ...
    Widget selectedItemWidget() {
      ...
      return Text(_selectedItemAsString(getSelectedItem), style: Theme.of(context).textTheme.titleMedium);
    }
  ...
  InputDecoration _manageDropdownDecoration(FormFieldState state) {
    return (widget.dropdownSearchDecoration ?? InputDecoration(contentPadding: EdgeInsets.fromLTRB(12, 12, 0, 0), border: OutlineInputBorder())).applyDefaults(Theme.of(state.context).inputDecorationTheme).copyWith(enabled: widget.enabled, labelText: widget.label ?? widget.dropdownSearchDecoration?.labelText, hintText: widget.hint ?? widget.dropdownSearchDecoration?.hintText, suffixIcon: widget.showAsSuffixIcons ? _manageTrailingIcons() : null, errorText: state.errorText);
  }
  ...
  Future _openMenu() {
    ...
    return customShowMenu<T>(popupSafeArea: widget.popupSafeArea, barrierColor: widget.popupBarrierColor, shape: widget.popupShape, color: widget.popupBackgroundColor, context: context, position: position, elevation: widget.popupElevation, barrierDismissible: widget.popupBarrierDismissible, items: [
      CustomPopupMenuItem(
        child: Container(
